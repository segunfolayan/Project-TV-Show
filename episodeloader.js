...
async function fetchEpisodes(showId) {
  ...
  const endpoint = `https://api.tvmaze.com/shows/${showId}/episodes`  //Defines the source of the data
...
async function setup() {
  ...
  const selectedId = urlParams.get("showId");
 

  showStatusMessage("Loading episodes, please wait...", "#fff3cd"); //show this message while fetching episodes
  const allEpisodes = await fetchEpisodes(selectedId); //Fetch episodes
